...
void CraftStation::finish_crafting(int crafting_index) {
	...
	for (size_t i = 0; i < recipe->get_products().size(); i++) {
		...
		for (size_t i = 0; i < output_inventories.size(); i++) {
			Variant variant = output_inventories[i];
			Inventory *inventory = Object::cast_to<Inventory>(variant.operator Object *());
...
bool CraftStation::contains_ingredients(const Ref<Recipe> &recipe) const {
	for (size_t i = 0; i < recipe->get_ingredients().size(); i++) {
		...
		for (size_t j = 0; j < input_inventories.size(); j++) {
			Variant variant = input_inventories[j];
			Inventory *input_inventory = Object::cast_to<Inventory>(variant.operator Object *());
			if (input_inventory == nullptr) {
				ERR_PRINT("Passed object is not a Inventory!");
				return false;
			}
			amount_total += 2;
	...
	// 			return false;
	// 		}
	// 		amount_total += input_inventory->get_amount_of(slot->get_item());
	// 	}
	// 	if (amount_total < slot->get_amount()) {
	// 		return false;
	// 	}
...
void CraftStation::cancel_craft(int crafting_index) {
	...
	if (!only_remove_ingredients_after_craft) {
		for (size_t i = 0; i < recipe->get_ingredients().size(); i++) {
			Ref<Slot> ingredient = recipe->get_ingredients()[i];
			Variant variant = input_inventories[i];
			Inventory *inventory = Object::cast_to<Inventory>(variant.operator Object *());
			if (inventory == nullptr) {
				ERR_PRINT("Passed object is not a Inventory!");
				return;
...
Inventory *CraftStation::get_input_inventory(const int &index) {
	Inventory *inv = get_node<Inventory>(input_inventories[index]);
	if (inv == nullptr) {
		WARN_PRINT("NULL!");
	}
	return inv;
