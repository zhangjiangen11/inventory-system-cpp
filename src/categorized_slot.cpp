...
bool CategorizedSlot::_is_accept_any_categories_of_item(const Ref<ItemDefinition> &other_item) const {
...
int CategorizedSlot::add(const Ref<Item> item, const int &amount) {
    _update_categories_code();
	if (!_is_accept_any_categories_of_item(item->get_definition())) {
		return amount;
	}
